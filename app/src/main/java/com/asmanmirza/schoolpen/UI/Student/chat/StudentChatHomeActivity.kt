...
import com.asmanmirza.schoolpen.databinding.ActivityStudentChatHomeBinding
import com.asmanmirza.schoolpen.presentation.main.Teacher.chat.AdapterTeachersChat
import com.google.android.material.tabs.TabLayout

class StudentChatHomeActivity : AppCompatActivity() {
    lateinit var binding: ActivityStudentChatHomeBinding
    lateinit var adapterTeachersChat: AdapterTeachersChat;

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityStudentChatHomeBinding.inflate(layoutInflater)
    ...
    private fun updateData() {
        ...
        binding.apply {
            ...
            recTeachersChat.layoutManager =
                LinearLayoutManager(
                    this@StudentChatHomeActivity,
                    LinearLayoutManager.VERTICAL,
                    false
                )
            adapterTeachersChat = AdapterTeachersChat(this@StudentChatHomeActivity, getChats());
            ...
            tabs.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
                override fun onTabSelected(tab: TabLayout.Tab?) {
                    when (tab?.position) {
                        ...
                        0 -> {
                            adapterTeachersChat =
                                AdapterTeachersChat(this@StudentChatHomeActivity, getChats())
                            recTeachersChat.adapter = adapterTeachersChat
                        }
                        1 -> {
                            ...
                            for (i in 0 until data.size) {
                                ...
                                if (!md.isTeacher) {
                                    temp.add(md)
                                    adapterTeachersChat =
                                        AdapterTeachersChat(this@StudentChatHomeActivity, temp)
                                    recTeachersChat.adapter = adapterTeachersChat
                                }
                            }
                        }
            ...
            ItemClickSupport.addTo(recTeachersChat)
                .setOnItemClickListener { recyclerView, position, v ->
                    if (position == 0) {
                        startActivity(
                            Intent(
                                this@StudentChatHomeActivity,
                                StudentChatDetailActivity::class.java
                            )
                        )
                    } else {
                        Toast.makeText(
